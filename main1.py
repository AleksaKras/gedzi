...
def category(query):
    ...
    Завдання квесту – це дати 7-8 правильних відповідей на запитання з Telegram-боту.
...
\nШукай наступну підказку у місці збору тролів…</i>
        ...
        bot.send_message(query.message.chat.id,
                         '''
<b>Підказка, де шукати першу легенду:</b>
\n<i>У лісі є тунель магічний таємничий, 
Пройдеш крізь нього, мрію в серці збережи.
Одне бажання загадай — і знай, що враз, 
Воно здійсниться в цю ж мить, в цей час.
\nШукай наступну підказку у місці де усі загадують бажання.</i>
                         ''', parse_mode='html')
        index_of_category = 1
    elif query.data == "cat3":
        bot.send_message(query.message.chat.id, '''
<b>Підказка, де шукати першу легенду:</b>
\n<i>Ельфи ткали ночі шовк й сплели з місячних ниток,
Кільце чарівне створили, срібний дарунок предивний. 
Через нього видно світ, де мрії оживають,
В казку вічну, світлу, нову серце поринає.
...
''', parse_mode='html')
        ...
        bot.send_message(query.message.chat.id, '''
    <b>Підказка, де шукати першу легенду:</b>
    \n<i>Хобіт Борфей біля озера жив,  
З вудкою завжди, рибалити любив.  
З водяними створіннями дружбу він вів,  
Допомагав їм завжди, де б не був чи ходив
\nШукай кюар код там, де грибами можна полікувати усе на світі.</i>
    ''', parse_mode='html')
        ...
        bot.send_message(query.message.chat.id, '''
    <b>Підказка, де шукати першу легенду:</b>
    \n<i>На високому дереві, поруч де хобіти живуть, 
Зелені папуги Лемонти мирно свій день прядуть. 
Серед шепоту листя, пісні вітру летять, 
Лемонти веселково в цій казці дзвенять.
\nШукай наступну легенду там, де зелені крила мерехтять.
</i>
    ''', parse_mode='html')
        index_of_category = 4
